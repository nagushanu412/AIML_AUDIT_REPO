...
import { ACCEPTED_FILE_TYPES, MANDATORY_COLUMNS } from "@/lib/procurement-testing/constants";
import type { UploadedFileInfo } from "@/lib/procurement-testing/types";
import { formatFileSize, formatRecordCount } from "@/lib/journal-entry-testing/utils";
import { cn } from "@/lib/utils/cn";

interface ProcurementFileUploadSectionProps {
  file: UploadedFileInfo | null;
  onFileChange: (file: UploadedFileInfo | null) => void;
  disabled?: boolean;
}

export function ProcurementFileUploadSection({
  file,
  onFileChange,
  disabled = false,
}: ProcurementFileUploadSectionProps) {
  ...
  const processFile = useCallback(
    (raw: File) => {
      ...
      if (!raw.name.toLowerCase().endsWith(".xlsx")) {
        setError("Only .xlsx vendor invoice register files are supported.");
        return;
      }
      onFileChange({
        name: raw.name,
  ...
  return (
    <SectionCard
      title="Vendor Invoice Register Upload"
      description="Upload vendor invoice register (.xlsx) for enterprise accounts payable substantive testing."
      ...
      {!file ? (
        ...
        >
          ...
          <p className="mt-4 text-sm font-medium text-slate-900 dark:text-white">
            Drag and drop vendor invoice register (.xlsx)
          </p>
          <Button
            type="button"
            variant="primary"
          ...
          >
            Upload vendor invoice register
          </Button>
          <p className="mt-3 text-xs text-slate-500">
            Sample: database/sample_procurement_invoices.xlsx
          </p>
        </div>
      ) : (
        <div className="rounded-xl border border-teal-100 bg-teal-50/30 p-4 dark:border-teal-900/50">
